...
test('supports direct canvas editing and isometric preview in the unified cockpit', async ({ page }) => {
  ...
  await expect.poll(() => getZoomPercent(page)).toBeGreaterThanOrEqual(101)
  ...
  await expect.poll(() => getZoomPercent(page)).toBeLessThanOrEqual(100)
  ...
  await expect
    .poll(async () => Number.parseInt((await page.locator('.toolbar-pill').first().textContent()) ?? '0', 10))
  ...
  await expect(page.getByLabel('Isometric preview')).toBeVisible()
  await expect(page.getByText('1 room across 1 floor')).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(page.getByLabel('Interactive floorplan canvas')).toBeVisible()

  await page.getByRole('button', { name: 'House view' }).click()
  await page.getByRole('tab', { name: 'Preview / Export' }).click()
  await page.getByRole('button', { name: 'Preview isometric' }).click()
  await expect(page.getByLabel('Isometric preview')).toBeVisible()
  await expect(page.getByText('5 rooms across 2 floors')).toBeVisible()
...
test('supports right-click menus across the 2D view and JSON round-trips', async ({ page }) => {
  ...
  await expect(page.getByRole('menu')).toContainText('Preview whole house')
  ...
  await expect(page.getByRole('menu')).toContainText('Preview floor')
  await page.mouse.click(10, 10)
  ...
  await expect(page.getByRole('menu')).toContainText('Preview room')
  await page.mouse.click(10, 10)
  ...
  await expect(canvasMenu).toContainText('Preview current scope')
  ...
  await page.locator('[data-testid^="furniture-label-"]').first().click({ button: 'right' })
  await expect(page.getByRole('menu')).toContainText('Edit furniture')
  await expect(page.getByRole('menu')).toContainText('Preview room')
  await page.mouse.click(10, 10)
  ...
  await expect(page.getByTestId('structure-header').getByText('Cedar House')).toBeVisible()
})
...
test('measures multiple distances from the canvas context menu until cleared', async ({ page }) => {
  ...
  const inferenceToggle = page.getByRole('checkbox', { name: 'Inference' })
  await inferenceToggle.click()
  await expect(inferenceToggle).not.toBeChecked()

  await page.mouse.click(canvasBox.x + 260, canvasBox.y + 190, { button: 'right' })
  await page.getByRole('menuitem', { name: 'Measure From Here' }).click({ force: true })
  await expect(page.getByTestId('canvas-measurement-pending-label')).toBeVisible()

  await page.getByTestId('canvas-empty').click({ position: { x: 228, y: 150 } })
  await expect(measurementLabels).toHaveCount(1)
  await expect(measurementLines).toHaveCount(1)
  await expect(page.getByRole('dialog')).toHaveCount(0)

  await page.mouse.click(canvasBox.x + 196, canvasBox.y + 212, { button: 'right' })
  await page.getByRole('menuitem', { name: 'Measure From Here' }).click({ force: true })
  await expect(page.getByTestId('canvas-measurement-pending-label')).toBeVisible()

  await page.getByTestId('canvas-empty').click({ position: { x: 164, y: 278 } })
  await expect(measurementLabels).toHaveCount(2)
  await expect(measurementLines).toHaveCount(2)

  await page.getByTestId('canvas-empty').click({ button: 'right', position: { x: 274, y: 204 } })
  await page.getByRole('menuitem', { name: 'Clear All Measurements' }).click({ force: true })
...
async function getZoomPercent(page: Page) {
  return Number.parseInt((await page.locator('.toolbar-pill').first().textContent()) ?? '0', 10)
}
